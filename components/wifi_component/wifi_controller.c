    ...
    if (event_base == WIFI_PROV_EVENT) {
        switch (event_id) {
            ...
            case WIFI_PROV_CRED_FAIL: {
                 ...
                 * CONFIG_EXAMPLE_RESET_PROV_MGR_ON_FAILURE configuration. It
                 * allows the provisioning manager to retry the provisioning
                 * process based on the number of attempts specified in
                 * wifi_conn_attempts. After attempting the maximum number of
                 * retries, the provisioning manager will reset the state
                 * machine and the provisioning process will be terminated.
                 */
 ...
 * etc. Note that memory for the response buffer must be allocated using heap as
 * this buffer gets freed by the protocomm layer once it has been sent by the
 * transport layer.
 */
...
esp_err_t wifi_controller_connect(bool reprovision_override) {
    ...
    if (!provisioned) {
        ...
        /* The username must be the same one, which has been used in the
         * generation of salt and verifier */
...
void wifi_controller_reprovision(void) {
    ...
    ESP_LOGI(TAG, "Starting re-provisioning");
